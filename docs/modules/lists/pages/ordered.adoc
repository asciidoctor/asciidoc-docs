...
If you decide to manually number your ordered list, you have to keep the items sequential.
...
If your ordered list includes figures or independent chunks of text, as is often the case when documenting steps in a procedure, you may find your steps resetting to 1 when you don't want them to. 
This may happen, for example, after figures or multiparagraph steps. 
...
Instead, use a list continuation—a `{plus}` symbol on its own line—between block elements (multiple paragraphs, images, tables, source blocks, etc.) that are separated by blank lines within a list item, as shown here:
