...
****
When an ordered list item includes block content (e.g., images, tables, paragraphs, source blocks, empty lines, etc.), you may observe that the number of the next item in the list resets to 1.
In fact, what's happened is that a new list has been started due to a missing list continuation.

In these cases, you should not resort to using the `start` attribute to fix the numbering.
Not only does that require manual adjustment as items are added to the list, it doesn't address the underlying semantics problem of the list being separated.
...
The list continuation glues the blocks together within a given item and keeps them at the same level of indentation, as shown in this illustration:
...
The list continuations prevent step 2 from resetting to 1. They also prevent step 5, which is pulled in from a separate .adoc file, from resetting to 1.
****
