...
When your ordered list includes block elements—images, captions, code, tables, multiple paragraphs, source blocks, etc., as is often the case when you document steps in a procedure—your steps may reset to 1 when you don't want them to. 
...
Instead, use a list continuation—a `{plus}` symbol on its own line—between block elements. The list continuation glues the content elements together within a given step. If a step includes any blank lines between content elements, the next step goes back to 1. (A blank line directly before a step has no effect on step numbering.) The list continuation also keeps the block elements aligned at the same level of indent, as shown here:

image::step-numbers-list-continuation.png[Example of continuous step numbering with mixed content elements]
