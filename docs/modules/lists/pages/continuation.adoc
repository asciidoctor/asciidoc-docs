...
But as with regular paragraph text, the text in a list item can wrap across any number of lines, as long as all the lines are adjacent.
...
TIP: When items contain more than one line of text, leave a blank line before the next item to make the list easier to read while working in the code.
In ordered lists, blank lines between list items do not reset numbering to 1.
Numbering remains continuous unless something, such as a diagram or note, comes between the numbered items.
In that case, 
