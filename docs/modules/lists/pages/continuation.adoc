...
To keep numbering continuous in those cases—for example, if you're documenting steps in a procedure—use the list continuation—a `{plus}` symbol—in the otherwise blank line. 

A list item may contain any type of AsciiDoc content, including paragraphs, delimited blocks, and block macros.
You just need to attach that content to the list item by using a list continuation—a `{plus}` symbol on its own line.
...
NOTE: A `{plus}` symbol at the end of a line is not a list continuation. In that case, the `{plus}` simply creates a line break. 
