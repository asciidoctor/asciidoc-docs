...
This page covers how to make a list item that has attached blocks and/or mixed nesting, known as a [.term]*complex list item*.
The page also draws comparison to list items with principle text that's either empty or that spans multiple lines.
The main purpose of the syntax covered on this page is to keep the list continuous.
...
For ordered lists, that means the numbering will restart.

To keep the list, and thus the numbering, continuous in those cases--such as when you're documenting complex steps in a procedure--you must use a <<list-continuation,list continuation>> to attach blocks to the list item.

[#list-continuation]
== Attach blocks using a list continuation

In addition to the principal text, a list item may contain any type of AsciiDoc content, including paragraphs, delimited blocks, and block macros.
