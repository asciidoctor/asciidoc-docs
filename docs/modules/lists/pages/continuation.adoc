...
This section applies to complex lists—ordered or unordered lists that have multiple lines of text per item or that contain of mix of content types—that you want to keep continuous. 
...
To keep numbering continuous in those cases—for example, if you're documenting steps in a procedure—use the list continuation—a `{plus}` symbol—instead of a blank line. 

A list item may contain any type of AsciiDoc content, including paragraphs, delimited blocks, and block macros.
You just need to attach that content to the list item by using a list continuation—a `{plus}` symbol.
