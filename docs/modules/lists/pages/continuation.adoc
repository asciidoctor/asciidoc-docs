...
As with regular paragraph text, the text in a list item can wrap across any number of lines as long as all the lines are adjacent (no blank lines between them).
...
In ordered lists, blank lines between list items do not reset numbering to 1.
Numbering resets only when something, such as a figure or a text string, comes between the numbered items.
If you want numbering to continue in those cases—for example, if you're documenting steps in a procedure—use the list continuation symbol (`{plus}`) instead of a blank line. 
