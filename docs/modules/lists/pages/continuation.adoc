...
To keep numbering continuous in those cases—for example, if you're documenting steps in a procedure—put a `{plus}` symbol in the otherwise blank line. A `{plus}` used in this way is called a list continuation.
