...
Refer to xref:sections:ids.adoc[] for more information about how an AsciiDoc processor generates these IDs.
...
In AsciiDoc, the inline xref macro is used to create cross references (also called in-text or page citations) to content elements (sections, blocks, or phrases) that have an ID (regardless of whether that ID is explicit or auto-generated).
