...
IMPORTANT: Line numbering is added by the source highlighter, not the AsciiDoc converter.
Therefore, to get line numbering on a source block, you must have a source highlighter set and that source highlighter must support line numbering.
When using Asciidoctor, the only source highlighter that does not support line numbering is highlight.js.
