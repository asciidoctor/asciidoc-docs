...
They have boundaries that match the xref:text.adoc#formatting-marks-and-pairs[constrained and unconstrained formatting marks].
The main difference, however, is that they get applied first to surpress other formatting marks.

This type of passthrough is intended to surpress any special meaning of the source text itself.
...
As with all constrained pairs, the single plus passthrough is designed to be applied around a word or phrase.

----
A word or phrase between plus characters, such as +/document/{id}+, is not substituted.
You can also refer to formatting marks, like the backtick, using `+`+`.
However, special characters like +<+ and +>+ are still escaped in the output.
----

Being an uncontrained pair, the double plus passthrough can be applied anywhere in the text.

----
Text formatting is not applied to a link target if it is surrounded by double plus characters.
