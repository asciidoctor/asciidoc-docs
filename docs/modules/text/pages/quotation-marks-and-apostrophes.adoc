...
When entered using the kbd:[{apos}] and kbd:[{quot}] key, the AsciiDoc processor outputs straight quotation marks (also known as vertical or typewriter quotation marks).
However, by creating a pair of backticks (`{backtick}`) contained within a pair of single quotes (`{apos}`) or double quotes (`{quot}`), you can tell the AsciiDoc processor where to output curved quotation marks.
