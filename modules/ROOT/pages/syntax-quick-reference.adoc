...
NOTE: Including content from a URL is potentially dangerous, so it's disabled if the safe mode is SECURE or greater.
...
.Inline as monospace and literal (i.e., literal monospace)
[#ex-inline-literal]
----
...
.Masquerading open block
[#ex-masquerade]
----
include::blocks:example$open.adoc[tag=src]
----

.View result of <<ex-masquerade>>
[%collapsible.result]
====
include::blocks:example$open.adoc[tag=src]
====
