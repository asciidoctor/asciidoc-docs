...
:navtitle: AsciiDoc Syntax Reference
...
[IMPORTANT]
The examples on this page focus on the output produced for the html backend.
AsciiDoc is expected to complementary output when generating PDF, EPUB, and DocBook.
...
[#ex-lead]
----
include::text:example$text.adoc[tag=b-lead]
----

The default Asciidoctor stylesheet automatically styles the first paragraph of the preamble as a xref:blocks:preamble-and-lead.adoc[lead paragraph] if no role is specified on that paragraph.
...
[#ex-format]
----
include::text:example$text.adoc[tag=b-bold-italic-mono]
----

.View result of <<ex-format>>
[%collapsible.result]
====
include::text:example$text.adoc[tag=b-bold-italic-mono]
====
...
A xref:document:header.adoc[document header] is optional.
...
When the document type is `article` (the default), the document can only have one level-0 section title (`=`).
This level-0 section title (i.e., the document title) must be in the document header.

.Article doctype section levels
[#ex-article]
----
...
The presense of at least one part implicitly makes the document a multi-part book.

.Book doctype section levels
----
...
This feature enables a reader to easily bookmark the section.
...
When the document attribute `sectlinks` is set, the section titles become self-links.
This feature enables a reader to easily bookmark the section.
...
You can force two adjacent lists apart by inserting a blank line followed by a line comment after the first list.
...
The xref:lists:unordered.adoc#markers[unordered list marker] can be changed.
...
Checklists can use xref:lists:checklist.adoc[font-based icons and be interactive].
...
Images are resolved relative to the value of the xref:macros:images-directory.adoc[imagesdir] document attribute, which is empty by default.
Use the `imagesdir` attribute to avoid hard coding the common path to your images in every image macro.
...
////
from @graphitefriction: This example doesn't work (image isn't displayed, just its alt text). Need to fix.
...
.Inline as monospace and literal
[#ex-inline-literal]
----
...
[IMPORTANT]
.Enabling the source highlighter
====
xref:verbatim:source-highlighter.adoc[Source highlighting] is enabled by setting the `source-highlighter` attribute in the document header or passed as an argument.

 :source-highlighter: rouge

The built-in values are `coderay`, `highlightjs`, `rouge`, and `pygments`.
====
...
.Strip leading indentation from included file content
------
...
.Shorthand method for assigning block id and role
----
[#goals.incremental]
* Goal 1
* Goal 2
...
.Assigning id (anchor) and role to inline formatted text
----
